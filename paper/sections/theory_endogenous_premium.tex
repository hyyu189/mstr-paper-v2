...
\begin{definition}[Endogenous premium system]
  ...
  \begin{itemize}
    ...
    \item $V_t$ is the BTC daily trading volume and $\eta \ge 0$ is the
          market-impact coefficient (Kyle lambda; see
          Section~\ref{sec:market_impact} for the full treatment);
...
\begin{theorem}[Existence and multiplicity of equilibria]
  ...
  \begin{enumerate}
    ...
    \item \textbf{(Strong feedback)} For intermediate $\Gamma$, there
      exist \emph{two} fixed points:
      \begin{itemize}
        \item $\pi^\star_{\mathrm{low}}$: the \emph{low equilibrium}
              (stable, $G < 1$);
        \item $\pi^\star_{\mathrm{high}}$: the \emph{high equilibrium}
              (unstable, $G > 1$), which acts as the \emph{tipping point}
              separating the basin of attraction of $\pi^\star_{\mathrm{low}}$
              from explosive premium growth.
      \end{itemize}

    \item \textbf{(Bifurcation)} At a critical
      $\Gamma^* = e^{-(\sigma_\pi^{\mathrm{ss}})^2/2}$, the two fixed points
      merge and disappear: for $\Gamma > \Gamma^*$ no finite equilibrium
      exists under the steady-state formula, signalling that the feedback
      loop is too strong for any static equilibrium to be self-consistent.
  \end{enumerate}
\end{theorem}
...
\begin{proof}
  ...
  $F(\pi^\star) \sim \log\bigl(1 + \Gamma\,
  e^{\pi^\star + (\sigma_\pi^{\mathrm{ss}})^2/2}\bigr) \sim \pi^\star +
  \tfrac{1}{2}(\sigma_\pi^{\mathrm{ss}})^2 + \log\Gamma$,
  so $h(\pi^\star) \equiv F(\pi^\star) - \pi^\star \to
  \log\Gamma + \tfrac{1}{2}(\sigma_\pi^{\mathrm{ss}})^2$.

  \emph{Step 2: Shape of $h$.}
  Differentiating $F$ using the Black--Scholes delta identity shows
  $F'(\pi^\star) = \Gamma\,\Phi(d_2)\,/\,(1 + \Gamma\,C(\pi^\star))
  + 1 - 1$, where
  $C = \mathbb{E}[(e^\pi-1)^+]$ and $d_2 = \pi^\star/\sigma_\pi^{\mathrm{ss}}$.
  Setting $h'=0$ yields $\Gamma\,\Phi(d_2) = 1$, which has at most one
  solution.  Hence $h$ has at most one local extremum (a minimum), and
  $h = 0$ has at most two solutions under the steady-state formula
  alone.

  \emph{Step 3: Bifurcation.}
  For small $\Gamma$ (specifically $\Gamma < \Gamma^*$), $h$ is
  monotonically decreasing with $h(0)>0$ and
  $h(\infty) = \log\Gamma + \tfrac{1}{2}(\sigma_\pi^{\mathrm{ss}})^2 < 0$,
  yielding a unique zero.
  At $\Gamma = \Gamma^* = e^{-(\sigma_\pi^{\mathrm{ss}})^2/2}$,
  we have $h(\infty) = 0$: the fixed point escapes to infinity.
  For $\Gamma > \Gamma^*$ (but $\Gamma < 1$), $h$ remains monotonically
  decreasing with $h(\infty)>0$, so $h>0$ everywhere and no finite
  fixed point exists.

  \emph{Step 4: Stability.}
  At any fixed point, the linearized dynamics have eigenvalue
  $-\kappa(1 - F'(\pi^\star))$.  The fixed point is stable iff
  $F'(\pi^\star) < 1$, i.e., $G < 1$.  When two fixed points exist
  (for intermediate $\Gamma$ slightly below $\Gamma^*$),
  $\pi^\star_{\mathrm{low}}$ satisfies $F' < 1$ (stable), while
  $\pi^\star_{\mathrm{high}}$ satisfies $F' > 1$ (unstable).
\end{proof}

\begin{remark}[Economic interpretation of the equilibria]
  \label{rem:three_equil_econ}
  \leavevmode
  \begin{itemize}
    \item \textbf{Low equilibrium} ($\pi^\star_{\mathrm{low}}$):
      Low-to-moderate premium $\to$ measured issuance $\to$ steady accretion
      $\to$ justified premium.  In the limit, if the premium turns negative,
      this becomes the death spiral (Section~\ref{sec:death_spiral}).
    \item \textbf{Tipping point} ($\pi^\star_{\mathrm{high}}$, when it
      exists): Unstable separator.  Above it, the feedback loop is
      self-amplifying (the ``flywheel ignition'' threshold); below it, the
      system is attracted back to $\pi^\star_{\mathrm{low}}$.
    \item \textbf{No-equilibrium regime} ($\Gamma > \Gamma^*$):
      The feedback is too strong for any static premium to be self-consistent;
      the system either explodes upward (flywheel) or must be stabilized by
      forces outside the steady-state model (market impact, capacity
      constraints, or endogenous variance).
  \end{itemize}
\end{remark}

\begin{remark}[Three equilibria with endogenous variance]
  \label{rem:three_equil_endo}
  The steady-state formula~\eqref{eq:pi_star_closed} uses the ``bare''
  mean-reversion speed $\kappa$ in the stationary variance
  $(\sigma_\pi^{\mathrm{ss}})^2 = \sigma_\pi^2/(2\kappa)$.
  In the full nonlinear system, the effective variance is
  $\sigma_\pi^2/\bigl(2\kappa(1-G)\bigr)$, which increases with $G$.
  This endogenous variance introduces additional nonlinearity that can
  produce a third (flywheel) equilibrium at high $\pi^\star$, restoring
  the classic three-equilibrium picture: low (stable), tipping point
  (unstable), and flywheel (stable).  The qualitative economic intuition
  of Section~\ref{sec:multiple_equilibria} remains valid under this
...
\begin{proposition}[Hysteresis in premium transitions]
  ...
  %
  \begin{enumerate}
    \item As $\Gamma$ increases toward $\Gamma^*$, the stable fixed point
      $\pi^\star_{\mathrm{low}}$ rises continuously.  The system tracks
      this equilibrium smoothly until $\Gamma$ exceeds $\Gamma^*$, at
      which point no finite equilibrium exists and the premium enters an
      explosive (flywheel) regime.
    \item If the system is in the flywheel regime and $\Gamma$
      subsequently decreases below $\Gamma^*$ (e.g., due to rising $r$
      or falling $\alpha_N$), the finite equilibrium reappears at a
      \emph{high} $\pi^\star$ value, and the premium collapses
      discontinuously to $\pi^\star_{\mathrm{low}}$.
...
\begin{enumerate}
  ...
  \item The reflexive feedback loop creates multiple equilibria connected by
    bifurcations with hysteresis (Theorem~\ref{thm:three_equilibria},
    Proposition~\ref{prop:hysteresis}).  With endogenous variance, the
    classic flywheel/tipping-point/spiral triad emerges
    (Remark~\ref{rem:three_equil_endo}).
